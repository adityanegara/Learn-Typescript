...
const _12_Matrix_1 = require("../src/12.Matrix");
describe('Matrix', () => {
    it('extract row from one number matrix', () => {
        const expected = [1];
        expect(new _12_Matrix_1.Matrix('1').rows[0]).toEqual(expected);
    });
    it('can extract row', () => {
        const expected = [3, 4];
        expect(new _12_Matrix_1.Matrix('1 2\n3 4').rows[1]).toEqual(expected);
    });
    it('extract column from one number matrix', () => {
        const expected = [1];
        expect(new _12_Matrix_1.Matrix('1').columns[0]).toEqual(expected);
    });
    it('can extract column', () => {
        const expected = [3, 6, 9];
        expect(new _12_Matrix_1.Matrix('1 2 3\n4 5 6\n7 8 9').columns[2]).toEqual(expected);
    });
    ...
    it('extract column where numbers have different widths', () => {
        ...
        expect(new _12_Matrix_1.Matrix('89 1903 3\n18 3 1\n9 4 800').columns[1]).toEqual(expected);
    });
});
